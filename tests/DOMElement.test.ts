...
describe('DOMElement Plugin', () => {
    ...
    it('supports siblings', () => {
        ...
        expect(parent).toPrettyPrintTo(
            ['<div>', '  <span>', '    some ', '  </span>', '  <span>', '    text', '  </span>', '</div>'].join('\n')
        );
    });
...
Testing.`;
        ...
        expect(textarea).toPrettyPrintTo(
            ['<textarea', '  name="tagline"', '>', '  Painless.', 'JavaScript.', 'Testing.', '</textarea>'].join('\n')
        );
    });
    ...
    it('supports empty text node', () => {
        ...
        expect(parent).toPrettyPrintTo(
            ['<span>', '  ', '  <abbr', '    title="meter"', '  >', '    m', '  </abbr>', '</span>'].join('\n')
        );
    });
    ...
    it('supports non-empty text node', () => {
        ...
        parent.innerHTML = ['<strong>Jest</strong>', ' means ', '<em>painless</em>', ' Javascript testing'].join('');
    ...
    describe('matches constructor name of SVG elements', () => {
        ...
        const expected = ['<svg', '  viewBox="0 0 1 1"', '>', '  <title>', '    JS community logo', '  </title>', '</svg>'].join(
            '\n'
        );
        ...
        test('jsdom 11', () => {
            ...
            const svg = new Element('svg', [{ name: 'viewBox', value: '0 0 1 1' }], title);
